    ...
    if not os.path.exists("../Project_data/graph/graph_routes.json"):
        ...
        for index, row in tqdm(data_routes.iterrows()):
            ...
            if response.status_code == 400:
                ...
                list = json_response["Not exactly 2 station given"]
                for nb_arrete in range(len(list) - 1):
                    graph.append(
                        {"route_id": route_id, "departure": list[nb_arrete], "destination": list[nb_arrete + 1],
                         "poids": random.random()})
            elif response.status_code == 200:
                json_response = json.loads(response.text)
                graph.append({"route_id": route_id, "departure": json_response["Departure"],
                              "destination": json_response["Destination"], "poids": random.random()})
